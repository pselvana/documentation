...
:::important
Use a signed certificate from a valid CA for production environments.
:::
...
:::note notes
- Because edits made to properties outside of the playbook may be overwritten by this procedure, these settings should be incorporated as `--extra-vars`. For example, `--extra-vars=enable_tls=true`. See [Custom configuration](#custom-configuration).
